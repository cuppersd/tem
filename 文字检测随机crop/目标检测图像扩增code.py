...
def show_pic(img, bboxes=None):
    ...
    输入:
        img:图像np.array格式
        bboxes:图像的所有boudning box list, 格式为[[x1, y1, x2, y2, x3, y3, x4, y4 ]....]
        names:每个box对应的名称
    '''
    if not isinstance(bboxes, np.ndarray):
        bboxes = np.array(bboxes)
    for point in bboxes.astype(np.int):
        cv2.line(img, tuple(point[0]), tuple(point[1]), (255, 0, 0), 2)
        cv2.line(img, tuple(point[1]), tuple(point[2]), (255, 0, 0), 2)
        cv2.line(img, tuple(point[2]), tuple(point[3]), (255, 0, 0), 2)
        cv2.line(img, tuple(point[3]), tuple(point[0]), (255, 0, 0), 2)
    cv2.imwrite('./random_crop/' + str(uuid.uuid1()) + '1.jpg', img)
...
class DataAugment():
    ...
    def add_padding(self, im: np.ndarray, text_polys: np.ndarray, input_size: numbers.Number) -> tuple:
        """
        对图片和文本框进行resize
        :param im: 图片
        :param text_polys: 文本框
        ...
        if max(h, w) > input_size:
            w_scale = input_size / max(h, w)
            h_scale = input_size / max(h, w)
            im = cv2.resize(im, (0, 0), fx=h_scale, fy=w_scale)
        else:
            w_scale = 1
            h_scale = 1

        im_padded = 127 * np.ones((input_size, input_size, c), dtype=np.uint8)
        h_, w_, _ = im.shape
    ...
    def test(self, im: np.ndarray, text_polys: np.ndarray):
        ...
        # show_pic(t_im, t_text_polys)

        # print('随机旋转')
        # t_im, t_text_polys = self.random_rotate_img_bbox(im, text_polys, 180)
        # print(t_im.shape, t_text_polys.dtype)
        # show_pic(t_im, t_text_polys)

        # print('随机裁剪') # 随机裁剪然后resize到指定的尺寸大小，不改变长宽比例
        t_im, t_text_polys = self.random_crop_img_bboxes(im, text_polys)
        t_im, t_text_polys = self.add_padding(t_im, t_text_polys, 736)
        show_pic(t_im, t_text_polys)

        # print('随机裁剪')
        # t_im, t_text_polys = self.random_crop_image_pse(im, text_polys, 736,)
        # print(t_im.shape, t_text_polys.dtype)
        # show_pic(t_im, t_text_polys)
        ...
        # print(t_im.shape, t_text_polys.dtype)
        # show_pic(t_im, t_text_polys)
        # show_pic(im, text_polys)

        # print('加噪声')
        # t_im = self.add_noise(im)
        # show_pic(t_im, text_polys)


aug = DataAugment()
img = cv2.imread("./images_validation/img_1134.jpg")
boxxx = []
f = open("./gt_validation/gt_img_1134.txt", 'r', encoding='utf-8').readlines()
for i in f:
    boxxx.append(np.array([float(x) for x in i.split(',')[:8]]).reshape(4, 2))
aug.test(img, np.array(boxxx))
aug.test(img, np.array(boxxx))
aug.test(img, np.array(boxxx))
aug.test(img, np.array(boxxx))
aug.test(img, np.array(boxxx))
